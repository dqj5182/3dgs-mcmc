...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from):
    ...
    # Storage for Gaussian positions
    positions_log = []
    log_frequency = 100
    ...
    for iteration in range(first_iter, opt.iterations + 1):        
        ...
        with torch.no_grad():
            ...
            # Log positions every N iterations
            if iteration % log_frequency == 0:  # Example: every 500 iterations
                positions_log.append(gaussians._xyz.detach().cpu().clone())
                print(f"Logged positions at iteration {iteration}")


    # Save positions log for visualization
    os.makedirs("logs", exist_ok=True)
    torch.save(positions_log, "logs/gaussian_positions_log.pt")
